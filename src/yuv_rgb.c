...
void yuv422_to_rgb24(unsigned char *yuv422, unsigned char *rgb24, int width, int height)
{
	...
	{
		/*rgb24[x] = yuv444[x] + 1402 * (yuv444[x + 2] - 128) / 1000;
		rgb24[x + 1] = yuv444[x] - 34414 * (yuv444[x + 1] - 128) / 100000 - 71414 * (yuv444[x + 2] - 128) / 100000;
		rgb24[x+2] = yuv444[x] + 1772 * (yuv444[x + 1] - 128) / 1000;*/

		int r = (int)yuv444[x] + 1402 * ((int)yuv444[x + 2] - 128) / 1000;
		int g = (int)yuv444[x] - 34414 * ((int)yuv444[x + 1] - 128) / 100000 - 71414 * ((int)yuv444[x + 2] - 128) / 100000;
		int b = (int)yuv444[x] + 1772 * ((int)yuv444[x + 1] - 128) / 1000;

		rgb24[x] = r;
		rgb24[x + 1] = g;
		rgb24[x + 2] = b;
		if (r>255)rgb24[x] = 255;
		if (r<0)rgb24[x] = 0;
		if (g>255)rgb24[x + 1] = 255;
		if (g<0)rgb24[x + 1] = 0;
		if (b>255)rgb24[x + 2] = 255;
		if (b<0)rgb24[x + 2] = 0;
	}
	free(yuv444);
}
...
void yuyv_rgb24_std(unsigned char* videoFrame,int width,int height, uint8_t * imageLine1) {
	...
	for (yy = 0; yy < (height); yy++)
	{
		for (xx = 0; xx < (width / 2); xx++)
		{
			...
			imageLine1[(bpl*y) + (Bpp*x / 2) + 2] = B;

			Y = videoFrame[width * 2 * y + x + 2];

			yuv_to_rgb_24(Y, U, V, &R, &G, &B);
			imageLine1[(bpl*y) + (Bpp*(x / 2 + 1))] = R;
			imageLine1[(bpl*y) + (Bpp*(x / 2 + 1)) + 1] = G;
			imageLine1[(bpl*y) + (Bpp*(x / 2 + 1)) + 2] = B;
		}
	}
