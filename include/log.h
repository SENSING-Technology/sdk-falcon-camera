...
/* log file level */
#define LOG_FILE_LEVEL LOG_INFO

/* log stream level */
#define LOG_STREAM_LEVEL LOG_INFO
...
#else
#define write_log(mode, lhs, level, format, ...) _log_write(mode, lhs, level, format, ##__VA_ARGS__)
#endif

typedef list_node log_t;

typedef list_node msg_list;

typedef struct data_msg {
  char log_msg[SINGLE_LOG_SIZE];
  uint32_t mode;
  uint32_t datalen;
  uint32_t level;
} data_msg;
 ...
 */
log_handle_t *create_log_file(const char *log_filename, size_t max_file_size, size_t max_file_bak, size_t max_iobuf_size);
...
/**
 * add file or stream handle to handle list.
 * @param plist - handle list.this parameter can be NULL.
 * @param hld - msg handle(file or stream).
 * @return handle list.if lhs is not NULL, return itself.
 */
msg_list *add_to_msg_list(msg_list *plist, void *hld);
