...
typedef struct
{
  ...
  unsigned int pointcloudthreadid;
#else
  pthread_t threadid;
  pthread_t depththreadid;
  pthread_t pointcloudthreadid;
#endif
...
void *pointcloudthread(void *param) {
  ...
  float* pointclouddata=(float*)calloc(1,sizeof(float)*4*1280*800);
  ...
  while (ptmpinfo->willrun) {
    ...
    if (p) {
      ...
      if (ptmpinfo->camparam->size() == 1) {
         ...
         /*   nvpfm_info_printf("param pc:%f,%f,size:%d,%d,real param:%f,%f\n",
                      theparam.left_ir_photocenter[0],
                      theparam.left_ir_photocenter[1],
                      depthheader->width,
                      depthheader->height,
                      photocenter[0],
                      photocenter[1]);
                      */
            ptmpinfo->depthtrans->compute_depth2pointcloud(depthdata, depthheader->width, depthheader->height,
                            (float *)pointclouddata, 4*sizeof(float), focus, photocenter);

            float* centerpoint=pointclouddata+(depthheader->height/2*depthheader->width+depthheader->width/2)*4;
            nvpfm_info_printf("centerpoint:x=%f,y=%f,z=%f\n",centerpoint[0],centerpoint[1],centerpoint[2]);
      }
    }
...
/*
  ...
  if ( ptmpinfo->depthdataqueue == NULL) {
    ...
    pthread_create(&ptmpinfo->pointcloudthreadid, NULL, pointcloudthread, ptmpinfo);
  }
  if ( ptmpinfo->camparam->size() == 1) {
  ...
  /* int width=depthheader->width;
   ...
   nvpfm::compute_depth2pseudo(depthdata,width,height,pseudo);
...
int main(int argc, const char *argv[])
{
  if (argc > 1)
  {
    printf("usb camera usage:./test_pointcloud\n");
    exit(0);
  }
